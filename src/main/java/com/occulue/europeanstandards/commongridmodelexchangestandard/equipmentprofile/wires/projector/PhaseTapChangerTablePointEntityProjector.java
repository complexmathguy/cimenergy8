...
public class PhaseTapChangerTablePointEntityProjector {
  ...
  /*
   * Add to the PhaseTapChangerTablePoint
   *
   * @param	parentID	UUID
   * @param	addTo		childType
   * @return	PhaseTapChangerTablePoint
   */
  public PhaseTapChangerTablePoint addToPhaseTapChangerTablePoint(
      UUID parentId, PhaseTapChangerTablePoint addTo) {
    LOGGER.info("handling AssignPhaseTapChangerTablePointToPhaseTapChangerTablePointEvent - ");

    PhaseTapChangerTablePoint parentEntity = repository.findById(parentId).get();
    PhaseTapChangerTablePoint child =
        PhaseTapChangerTablePointProjector.find(addTo.getPhaseTapChangerTablePointId());

    parentEntity.getPhaseTapChangerTablePoint().add(child);
  ...
  /*
   * Remove from the PhaseTapChangerTablePoint
   *
   * @param	parentID	UUID
   * @param	removeFrom	childType
   * @return	PhaseTapChangerTablePoint
   */
  public PhaseTapChangerTablePoint removeFromPhaseTapChangerTablePoint(
      UUID parentId, PhaseTapChangerTablePoint removeFrom) {
    LOGGER.info("handling RemovePhaseTapChangerTablePointFromPhaseTapChangerTablePointEvent ");

    PhaseTapChangerTablePoint parentEntity = repository.findById(parentId).get();
    PhaseTapChangerTablePoint child =
        PhaseTapChangerTablePointProjector.find(removeFrom.getPhaseTapChangerTablePointId());

    parentEntity.getPhaseTapChangerTablePoint().remove(child);
  ...
  public PhaseTapChangerTablePoint find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a PhaseTapChangerTablePoint - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<PhaseTapChangerTablePoint> findAll(FindAllPhaseTapChangerTablePointQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all PhaseTapChangerTablePoint - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final PhaseTapChangerTablePointRepository repository;
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTablePointEntityProjector.class.getName());
}
