...
public class DiscreteEntityProjector {
  ...
  /*
   * Add to the Discretes
   *
   * @param	parentID	UUID
   * @param	addTo		childType
   * @return	Discrete
   */
  public Discrete addToDiscretes(UUID parentId, Discrete addTo) {
    LOGGER.info("handling AssignDiscretesToDiscreteEvent - ");

    Discrete parentEntity = repository.findById(parentId).get();
    Discrete child = DiscreteProjector.find(addTo.getDiscreteId());

    parentEntity.getDiscretes().add(child);
  ...
  /*
   * Remove from the Discretes
   *
   * @param	parentID	UUID
   * @param	removeFrom	childType
   * @return	Discrete
   */
  public Discrete removeFromDiscretes(UUID parentId, Discrete removeFrom) {
    LOGGER.info("handling RemoveDiscretesFromDiscreteEvent ");

    Discrete parentEntity = repository.findById(parentId).get();
    Discrete child = DiscreteProjector.find(removeFrom.getDiscreteId());

    parentEntity.getDiscretes().remove(child);
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final DiscreteRepository repository;

  @Autowired
  @Qualifier(value = "discrete-entity-projector")
  com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.meas
          .DiscreteEntityProjector
      DiscreteProjector;
