...
public class SwitchItEntityProjector {
   ...
   * @return	SwitchIt
   */
  public SwitchIt assignOpen(UUID parentId, BooleanProxy assignment) {
    LOGGER.info("assigning Open as " + assignment.toString());

    SwitchIt parentEntity = repository.findById(parentId).get();
    assignment = BooleanProxyProjector.find(assignment.getBooleanProxyId());

    // ------------------------------------------
    // assign the Open to the parent entity
    // ------------------------------------------
    parentEntity.setOpen(assignment);
   ...
   * @return	SwitchIt
   */
  public SwitchIt unAssignOpen(UUID parentId) {
    SwitchIt parentEntity = repository.findById(parentId).get();

    LOGGER.info("unAssigning Open on " + parentEntity.toString());

    // ------------------------------------------
    // null out the Open on the parent entithy
    // ------------------------------------------
    parentEntity.setOpen(null);
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final SwitchItRepository repository;
