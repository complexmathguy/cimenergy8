...
public class VoltageAdjusterUserDefinedEntityProjector {
  ...
  // core constructor
  public VoltageAdjusterUserDefinedEntityProjector(
      VoltageAdjusterUserDefinedRepository repository) {
   ...
   */
  public VoltageAdjusterUserDefined assignProprietary(UUID parentId, BooleanProxy assignment) {
    LOGGER.info("assigning Proprietary as " + assignment.toString());

    VoltageAdjusterUserDefined parentEntity = repository.findById(parentId).get();
   ...
   * @return	VoltageAdjusterUserDefined
   */
  public VoltageAdjusterUserDefined unAssignProprietary(UUID parentId) {
    VoltageAdjusterUserDefined parentEntity = repository.findById(parentId).get();
  ...
  public VoltageAdjusterUserDefined find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a VoltageAdjusterUserDefined - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<VoltageAdjusterUserDefined> findAll(FindAllVoltageAdjusterUserDefinedQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all VoltageAdjusterUserDefined - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final VoltageAdjusterUserDefinedRepository repository;
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VoltageAdjusterUserDefinedEntityProjector.class.getName());
}
