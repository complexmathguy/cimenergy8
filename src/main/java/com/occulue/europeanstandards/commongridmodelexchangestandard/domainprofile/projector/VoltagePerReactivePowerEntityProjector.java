...
public class VoltagePerReactivePowerEntityProjector {
  ...
  public List<VoltagePerReactivePower> findAll(FindAllVoltagePerReactivePowerQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all VoltagePerReactivePower - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final VoltagePerReactivePowerRepository repository;
  ...
  private static final Logger LOGGER =
      Logger.getLogger(VoltagePerReactivePowerEntityProjector.class.getName());
}
