...
public class ActivePowerPerCurrentFlowEntityProjector {
  ...
  public ActivePowerPerCurrentFlow find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a ActivePowerPerCurrentFlow - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<ActivePowerPerCurrentFlow> findAll(FindAllActivePowerPerCurrentFlowQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all ActivePowerPerCurrentFlow - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final ActivePowerPerCurrentFlowRepository repository;
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ActivePowerPerCurrentFlowEntityProjector.class.getName());
}
